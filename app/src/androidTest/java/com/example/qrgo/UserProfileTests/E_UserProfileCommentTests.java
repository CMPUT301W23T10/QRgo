...
public class E_UserProfileCommentTests extends BaseUserProfileTest {
    @Test
    ...
    public void cleanup() {
        if (imei.equals("") && username.equals("")) {
            ...
            db.collection("Comments").whereEqualTo("commentQRCode", "123456").get().addOnCompleteListener(task -> {
                if (task.isSuccessful()) {
                    QuerySnapshot querySnapshot = task.getResult();
                    if (querySnapshot.size() == 1) {
                        DocumentSnapshot document = querySnapshot.getDocuments().get(0);
